...
module AbsGrammar (Tree(..), Literal, Expr, SepExprList, MarkExprList, Type, MarkTypeList, Pattern, MarkPatternList, FunDecl, Parameter, SepParameterList, IfElse, IfStmt, Stmt, Block, Program, Ident, Boolean, johnMajorEq, module ComposOp) where
...
data Tag = Literal_ | Expr_ | SepExprList_ | MarkExprList_ | Type_ | MarkTypeList_ | Pattern_ | MarkPatternList_ | FunDecl_ | Parameter_ | SepParameterList_ | IfElse_ | IfStmt_ | Stmt_ | Block_ | Program_ | Ident_ | Boolean_
...
type MarkPatternList = Tree MarkPatternList_
...
data Tree :: Tag -> * where
    ...
    LiteralIdent :: Ident -> Tree Literal_
    ...
    PatternVariable :: Ident -> Tree Pattern_
    PatternMutableVariable :: Ident -> Tree Pattern_
    PatternIgnore :: Tree Pattern_
    PatternTuple :: MarkPatternList -> Tree Pattern_
    MarkPatternNil :: Tree MarkPatternList_
    MarkPatternOne :: Pattern -> Tree MarkPatternList_
    MarkPatternMore :: Pattern -> MarkPatternList -> Tree MarkPatternList_
    MarkPatternHead :: Pattern -> Tree MarkPatternList_
    ...
    LetStmtStrict :: Pattern -> Type -> Expr -> Tree Stmt_
    LetStmt :: Pattern -> Expr -> Tree Stmt_
...
instance Compos Tree where
  compos r a f t = case t of
      ...
      LiteralIdent ident -> r LiteralIdent `a` f ident
      ...
      PatternVariable ident -> r PatternVariable `a` f ident
      PatternMutableVariable ident -> r PatternMutableVariable `a` f ident
      PatternTuple markpatternlist -> r PatternTuple `a` f markpatternlist
      MarkPatternOne pattern -> r MarkPatternOne `a` f pattern
      MarkPatternMore pattern markpatternlist -> r MarkPatternMore `a` f pattern `a` f markpatternlist
      MarkPatternHead pattern -> r MarkPatternHead `a` f pattern
      ...
      LetStmtStrict pattern type' expr -> r LetStmtStrict `a` f pattern `a` f type' `a` f expr
      LetStmt pattern expr -> r LetStmt `a` f pattern `a` f expr
...
instance Show (Tree c) where
  showsPrec n t = case t of
    ...
    LiteralIdent ident -> opar n . showString "LiteralIdent" . showChar ' ' . showsPrec 1 ident . cpar n
    ...
    PatternVariable ident -> opar n . showString "PatternVariable" . showChar ' ' . showsPrec 1 ident . cpar n
    PatternMutableVariable ident -> opar n . showString "PatternMutableVariable" . showChar ' ' . showsPrec 1 ident . cpar n
    PatternIgnore -> showString "PatternIgnore"
    PatternTuple markpatternlist -> opar n . showString "PatternTuple" . showChar ' ' . showsPrec 1 markpatternlist . cpar n
    MarkPatternNil -> showString "MarkPatternNil"
    MarkPatternOne pattern -> opar n . showString "MarkPatternOne" . showChar ' ' . showsPrec 1 pattern . cpar n
    MarkPatternMore pattern markpatternlist -> opar n . showString "MarkPatternMore" . showChar ' ' . showsPrec 1 pattern . showChar ' ' . showsPrec 1 markpatternlist . cpar n
    MarkPatternHead pattern -> opar n . showString "MarkPatternHead" . showChar ' ' . showsPrec 1 pattern . cpar n
    ...
    LetStmtStrict pattern type' expr -> opar n . showString "LetStmtStrict" . showChar ' ' . showsPrec 1 pattern . showChar ' ' . showsPrec 1 type' . showChar ' ' . showsPrec 1 expr . cpar n
    LetStmt pattern expr -> opar n . showString "LetStmt" . showChar ' ' . showsPrec 1 pattern . showChar ' ' . showsPrec 1 expr . cpar n
...
johnMajorEq (LiteralIdent ident) (LiteralIdent ident_) = ident == ident_
...
johnMajorEq (PatternVariable ident) (PatternVariable ident_) = ident == ident_
johnMajorEq (PatternMutableVariable ident) (PatternMutableVariable ident_) = ident == ident_
johnMajorEq PatternIgnore PatternIgnore = True
johnMajorEq (PatternTuple markpatternlist) (PatternTuple markpatternlist_) = markpatternlist == markpatternlist_
johnMajorEq MarkPatternNil MarkPatternNil = True
johnMajorEq (MarkPatternOne pattern) (MarkPatternOne pattern_) = pattern == pattern_
johnMajorEq (MarkPatternMore pattern markpatternlist) (MarkPatternMore pattern_ markpatternlist_) = pattern == pattern_ && markpatternlist == markpatternlist_
johnMajorEq (MarkPatternHead pattern) (MarkPatternHead pattern_) = pattern == pattern_
...
johnMajorEq (LetStmtStrict pattern type' expr) (LetStmtStrict pattern_ type'_ expr_) = pattern == pattern_ && type' == type'_ && expr == expr_
johnMajorEq (LetStmt pattern expr) (LetStmt pattern_ expr_) = pattern == pattern_ && expr == expr_
...
index (LiteralIdent _) = 2
index (Assign _ _) = 3
index (Or _ _) = 4
index (And _ _) = 5
index (Equal _ _) = 6
index (NotEqual _ _) = 7
index (Less _ _) = 8
index (Add _ _) = 9
index (Subtract _ _) = 10
index (Multiply _ _) = 11
index (Divide _ _) = 12
index (Modulo _ _) = 13
index (Negate _) = 14
index (Dereference _) = 15
index (Not _) = 16
index (Borrow _) = 17
index (MutableBorrow _) = 18
index (LiteralExpr _) = 19
index (ExprIdent _) = 20
index (FunctionCall _ _) = 21
index (ArrayLookup _ _) = 22
index (TupleLookup _ _) = 23
index (IfElseExpr _) = 24
index (BlockExpr _) = 25
index (ArrayElements _) = 26
index (ArrayRepeat _ _) = 27
index (ArrayRange _ _) = 28
index (TupleConstruct _) = 29
index (SepExprNil ) = 30
index (SepExprOne _) = 31
index (SepExprMore _ _) = 32
index (SepExprHead _) = 33
index (SepExprTail _ _) = 34
index (MarkExprNil ) = 35
index (MarkExprOne _) = 36
index (MarkExprMore _ _) = 37
index (MarkExprHead _) = 38
index (MarkExprTail _ _) = 39
index (Bool ) = 40
index (I32 ) = 41
index (Reference _) = 42
index (MutableReference _) = 43
index (Array _ _) = 44
index (Tuple _) = 45
index (MarkTNil ) = 46
index (MarkTOne _) = 47
index (MarkTMore _ _) = 48
index (MarkTHead _) = 49
index (MarkTTail _ _) = 50
index (PatternVariable _) = 51
index (PatternMutableVariable _) = 52
index (PatternIgnore ) = 53
index (PatternTuple _) = 54
index (MarkPatternNil ) = 55
index (MarkPatternOne _) = 56
index (MarkPatternMore _ _) = 57
index (MarkPatternHead _) = 58
index (MarkPatternTail _ _) = 59
...
index (IfStmt _ _) = 69
index (FunDeclStmt _) = 70
index (Stmt _) = 71
index (Break ) = 72
index (Continue ) = 73
index (If _) = 74
index (IfElseStmt _) = 75
index (Loop _) = 76
index (While _ _) = 77
index (IterableForLoop _ _ _) = 78
index (RangeForLoop _ _ _ _) = 79
index (LetStmtStrict _ _ _) = 80
index (LetStmt _ _) = 81
index (BlockStmt _) = 82
index (Block _) = 83
index (BlockWithValue _ _) = 84
index (Program _) = 85
index (Ident _) = 86
index (Boolean _) = 87
...
compareSame (LiteralIdent ident) (LiteralIdent ident_) = compare ident ident_
...
compareSame (PatternVariable ident) (PatternVariable ident_) = compare ident ident_
compareSame (PatternMutableVariable ident) (PatternMutableVariable ident_) = compare ident ident_
compareSame PatternIgnore PatternIgnore = EQ
compareSame (PatternTuple markpatternlist) (PatternTuple markpatternlist_) = compare markpatternlist markpatternlist_
compareSame MarkPatternNil MarkPatternNil = EQ
compareSame (MarkPatternOne pattern) (MarkPatternOne pattern_) = compare pattern pattern_
compareSame (MarkPatternMore pattern markpatternlist) (MarkPatternMore pattern_ markpatternlist_) = mappend (compare pattern pattern_) (compare markpatternlist markpatternlist_)
compareSame (MarkPatternHead pattern) (MarkPatternHead pattern_) = compare pattern pattern_
...
compareSame (LetStmtStrict pattern type' expr) (LetStmtStrict pattern_ type'_ expr_) = mappend (compare pattern pattern_) (mappend (compare type' type'_) (compare expr expr_))
compareSame (LetStmt pattern expr) (LetStmt pattern_ expr_) = mappend (compare pattern pattern_) (compare expr expr_)
